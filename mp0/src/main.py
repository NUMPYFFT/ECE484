...
def run_model(d_safe = 15,v_0 = 10, a_b = 5, t_react = 0):
    ...
    while not rospy.is_shutdown():
        ...
        #print(targetState.pose.position.x, currState.pose.position.x)
        
        t.append(rospy.get_rostime())
        ...
        print(currState.pose.position.x)
...
def plot(x1,x2,t):
    t = np.arange(len(x1))
    plt.figure()
    plt.plot(np.array(t),np.array(x1))
    plt.plot(np.array(t),np.array(x2))
    plt.xlabel('time')
    plt.ylabel('distance')
...
def plot_Q6(x1,x2,t):
    t = np.arange(len(x1))
    plt.figure()
    plt.plot(np.array(t), abs(np.array(x2) - np.array(x1)))
    plt.xlabel('time')
    plt.ylabel('distance')
    #plt.legend(['x_1(t)', 'x_2(t)']) 
    plt.title('aaa100')
