...
def run_model():
    ...
    while not rospy.is_shutdown():
        ...
        xlist.append(currState.pose.position.x)
        ylist.append(currState.pose.position.y)
        print(len(xlist), len(pos_list))
    
        # if target_x == pos_list[-1][0] and target_y == pos_list[-1][1]:
        if len(xlist) > 14000:
            plt.figure()
