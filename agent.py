...
class Agent():
    ...
    def sample_points(self, curr_x, curr_y, target_x, target_y, robot_radius, obs_x, obs_y, obs_KDTree, rng, transform):
        ...
        if max_x_y - min_y == 0 or max_x - min_y_x == 0:
            ...
            theta = np.arctan((max_x_y - min_y) / (max_x - min_y_x))
    ...
    def run_step(self, filtered_obstacles, waypoints, vel, transform, boundary):
        ...
        if self.mode == 1:
            ...
            if filtered_obstacles != []:
                ...
                if obs_distance <= 12: # time to do avoiding ///15: the distance from the closest obstacle to start avoiding program
                    print('1111111111111')  
                    
                    for i in range(len(boundary[0])):
                        obs_x.append(-boundary[0][i].transform.location.x)
                    ...
                    for j in range(len(boundary[1])):
                        obs_x.append(-boundary[1][j].transform.location.x)
                    ...
                    self.paths = self.prm(curr_x=curr_x, curr_y=curr_y, target_x=target_x, target_y=target_y, robot_radius=5, transform=transform, obs_x=obs_x, obs_y=obs_y)
                    #self.paths = self.prm(curr_x=target_x, curr_y=target_y, target_x=curr_x, target_y=curr_y, robot_radius=5, transform=transform, obs_x=obs_x, obs_y=obs_y)
                    ...
                    if len(self.paths) != 0:
                        ...
                        plt.scatter(plot_x[1:-1], plot_y[1:-1], color='blue') #edges
                        # plt.scatter(plot_x[0], plot_y[0], color='green')
                        plt.scatter(plot_x[0], plot_y[0], color='green') # goal point
                    ...
                    self.using_waypoints = waypoints
            else:
                self.using_waypoints = waypoints
        ...
        if self.mode == 0:
            ...
            self.using_waypoints = copy.deepcopy(waypoints)
            self.using_waypoints[0] = self.paths[path_len-1-self.counter]
            #print("using waypoints", self.using_waypoints, "``````````````````````")
            curr_x, curr_y, curr_z = -transform.location.x, transform.location.y, transform.location.z
            #print("curr_x3 = ", curr_x)
            target_x, target_y, target_z = self.using_waypoints[0]
        ...
        curr_x, curr_y, curr_z = -transform.location.x, transform.location.y, transform.location.z
        #print("curr_x4 = ", curr_x)
        target_x, target_y, target_z = self.using_waypoints[0]
        #print("target_x3 = ", target_x)
        target_orientation = np.arctan((target_y - curr_y) / (target_x - curr_x)) #theta_ref
        #print("ty, cy, tx, cx", target_y, curr_y, target_x, curr_x)
        print("target_o", target_orientation)
        target_vel = 45
        target_vx = 0.01 * np.cos(target_orientation) * target_vel
        target_vy = 0.01 * np.sin(target_orientation) * target_vel
        target_x += target_vx
        #print("target_x4 = ", target_x)
        target_y += target_vy
        ...
        # added obstacles
        brake = self.min_radius / self.findCircle(self.using_waypoints[1:4])
        if brake == 1:
            brake = 0.2
        ...
        threshold = 3
        next_x, next_y, next_z = self.using_waypoints[1]
        dist2wp = np.sqrt((curr_x - next_y) ** 2 + (curr_y - next_y) ** 2)
